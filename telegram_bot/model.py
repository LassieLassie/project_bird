...
class ClassPredictor:
    ...
    def predict(self, img_stream):
        ...
        # посмотрим распознает ли изображение наша сеть
        image = self.process_image(img_stream).to(self.device).unsqueeze(0)
        output = model(image) # загоняем в модель
        ...
        return classes[pred.item()]###self.model.predict(self.process_image(img_stream))[0]
